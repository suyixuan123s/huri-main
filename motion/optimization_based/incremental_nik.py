...
class IncrementalNIK(object):
    ...
    def gen_linear_motion(self,
        ...
        self.robot_s.fk(component_name, jnt_values_bk)
        # check continuous of the motion
        if len(jnt_values_list) > 1:
            tmp_array = np.asarray(jnt_values_list)
            if len(np.where(np.linalg.norm(tmp_array[1:] - tmp_array[:-1], axis=1) > np.pi / 2)[0]) > 0:
                return None
