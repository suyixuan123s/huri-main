...
class PhxClient(object):
    ...
    def __init__(self, host="localhost:18300"):
        # Address problem of `gRPC: Received message larger than max`
    ...
    def getextcamimg(self):
        extcamimg = self.stub.getextcamimg(pxmsg.Empty())
        extcamimgarray = self._unpackarraydata(extcamimg)

        return extcamimgarray.astype(np.uint8)

    def changeledpower(self, ledpower):
        assert 0 <= ledpower <= 4095
        self.stub.changeledpower(pxmsg.PhoxiSettings(ledpower = ledpower))
