...
public:
	...
	// added by chen 20220616
	void changeledpower(int ledpower) {
		if (ledpower < 0 || ledpower > 4095) {
			std::cout << "LED Power should be within [0, 4095]" << std::endl;
			return;
		}
		std::cout << "Change Capturing Settings" << std::endl;
		std::cout << std::endl;
		//Retrieving the Current Capturing Settings
		pho::api::PhoXiCapturingSettings CurrentCapturingSettings;
		CurrentCapturingSettings = cam->CapturingSettings;
		//Check if the CurrentCapturingSettings have been retrieved succesfully
		if (!cam->CapturingSettings.isLastOperationSuccessful()) {
			throw std::runtime_error(cam->CapturingSettings.GetLastErrorMessage().c_str());
		}
		if (ledpower != cam->CapturingSettings->LEDPower)
			cam->CapturingSettings->LEDPower = ledpower;

		std::cout << "Change the LED power to " << ledpower << std::endl;
	}
	// added by chen 20230814
	void get_intrinsic_mat() {
		// Camera matrix
		for (std::size_t r = 0; r < 3; ++r)
		{
			for (std::size_t c = 0; c < 3; ++c)
			{
				...
				std::cout << "  " << std::setw(10) << std::fixed << std::setprecision(3)
					<< cam->CalibrationSettings->CameraMatrix.At(r, c);
			}
			std::cout << std::endl;
		}
		std::cout << std::endl;
		std::cout << "distortion information : ";
		auto distortion_info = cam->CalibrationSettings->DistortionCoefficients;
		for (auto i : distortion_info)
			std::cout << i << ' ';
	}
...
PYBIND11_MODULE(phoxicontrol, m) {
	py::class_<PhoxiControl>(m, "PhoxiControl")
		...
		.def("changeledpower", &PhoxiControl::changeledpower)
		.def("get_intrinsic_mat", &PhoxiControl::get_intrinsic_mat);
