...
class ManualCalibrationBase(ABC):
    ...
    def rotate_adjust(self, dir, dir_global, key_name=None):
        ...
        :return:
        """
        self._init_calib_mat[:3, :3] = np.dot(rm.rotmat_from_axangle(dir, np.radians(self.rotation_resolution)),
    ...
    def plot(self, task=None):
        ...
        self._plot_node_rbt = self._rbt_s.gen_meshmodel(rgba=[1, 1, 0, .6])
    ...
    def adjust(self, task):
        ...
        self.plot()
        self.save()
        print(repr(self._init_calib_mat))
        return task.again
...
class XArmLite6ManualCalib(ManualCalibrationBase):
    ...
    def move_adjust(self, dir, dir_global, key_name=None):
        ...
        rbt_pose = self._rbt_x.get_pose()
        w2r_mat = rm.homomat_from_posrot(*rbt_pose)
        w2r_mat_inv = np.linalg.inv(w2r_mat)
        self._init_calib_mat[:3, 3] = self._init_calib_mat[:3, 3] + rm.homomat_transform_points(w2r_mat_inv,
                                                                                                dir_global) * self.move_resolution
    ...
    def rotate_adjust(self, dir, dir_global, key_name=None):
        ...
        r2cam_mat = self._init_calib_mat
        w2r_mat = rm.homomat_from_posrot(*rbt_pose)
        r2w_mat = rm.homomat_inverse(w2r_mat)
        w2c_mat = w2r_mat.dot(r2cam_mat)
        w2c_mat[:3, 3] = w2c_mat[:3, 3] + dir_global * self.move_resolution

        w2c_mat[:3, :3] = np.dot(rm.rotmat_from_axangle(dir_global,
                                                        np.radians(self.rotation_resolution)),
                                 w2c_mat[:3, :3])

        self._init_calib_mat = np.dot(r2w_mat, w2c_mat)
...
if __name__ == "__main__":
    ...
    from huri.work3.dual_d405_manager import DEVICE_1_ID, DEVICE_2_ID
    from huri.work3.constants import TCP2EYE_MAT_DEVICE_1, TCP2EYE_MAT_DEVICE_2

    mode = 1
    if mode == 1:
        device = DEVICE_1_ID
        mat = TCP2EYE_MAT_DEVICE_1
        device2 = DEVICE_2_ID
        mat2 = TCP2EYE_MAT_DEVICE_2
    else:
        device = DEVICE_2_ID
        mat = TCP2EYE_MAT_DEVICE_2
        device2 = DEVICE_1_ID
        mat2 = TCP2EYE_MAT_DEVICE_1

    # device_2 = DEVICE_1_ID
    # mat2=

    base = wd.World(cam_pos=[2, 0, 1.5], lookat_pos=[0, 0, 0])
    rs_pipe = RealSenseD405(resolution='high', device=device)
    rs_pipe_2 = RealSenseD405(resolution='high', device=device2)
    ...
    rs_pipe_2.get_pcd_texture_depth()
    rs_pipe_2.get_pcd_texture_depth()
    pcd, pcd_color, _, _ = rs_pipe_2.get_pcd_texture_depth()
    pcd_color_rgba = np.append(pcd_color, np.ones((len(pcd_color), 1)), axis=1)
    p = rbtx.get_pose()
    rbt_mat = rm.homomat_from_posrot(*p)
    pcd_r = rm.homomat_transform_points(rbt_mat.dot(mat2), pcd)
    # gm.gen_pointcloud(pcd_r, rgbas=pcd_color_rgba).attach_to(base)
    ...
    xarm_mc = XArmLite6ManualCalib(rbt_s=rbt, rbt_x=rbtx, sensor_hdl=rs_pipe, init_calib_mat=mat,
                                   rotation_resolution=np.radians(5), move_resolution=.0001)
    base.run()
    rs_pipe.stop()
    rs_pipe_2.stop()
