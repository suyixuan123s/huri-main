...
DEBUG = False
from huri.work3.constants import HEIGHT_RANGE
...
def test(base):
    ...
    # (pcd_1, im_1), (pcd_2, im_2), (pcd_3, im_3), (pcd_4, im_4) = fs.load_pickle("vis_calibration_debug")
    (pcd_1, im_1), (pcd_2, im_2), (pcd_3, im_3), (pcd_4, im_4) = fs.load_pickle("../pcd_data_debug")
    # gm.gen_pointcloud(c_pcd, [[0, 0, 0, .3]]).attach_to(base)
    # Detect

    c_pcd = np.vstack((pcd_1, pcd_2, pcd_3, pcd_4))
    gm.gen_rgb_pointcloud(c_pcd).attach_to(base)
    pcd_ind = pntu.extract_pcd_by_range(pcd=c_pcd[:, :3],
                                        x_range=[.08, .4],
