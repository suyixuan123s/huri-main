...
from vision_sys import Rack_Locator, detect_tube, is_pose_err
...
from constants import (TCP2EYE_MAT,
                       HEIGHT_RANGE,
                       GOAL_PATTERN,
                       GOAL_UP_DISTANCE,
...
def init_vision_system(rl, detector, rbtx, add_to_scene_func=None, toggle_debug=False):
    ...
    for pcd, im in [(pcd_rgba, im_center), (pcd_1, im_1), (pcd_2, im_2)]:
        ...
        add_to_scene_func(gm.gen_rgb_pointcloud(pcd_rgba))
...
class Execution(object):
    def __init__(self):
        ...
        rl = Rack_Locator(rbthp.rbt_con, rs_pipe,
                          origin_obs_pose=np.array([0.17, 0., 0.12]),
                          hand_to_eye_mat=TCP2EYE_MAT,
                          toggle_debug=False)
        self.rl = rl
