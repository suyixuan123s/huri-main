...
class Rack_Locator(object):
    ...
    def _to_w_coord(self, pcd: np.ndarray, toggle_debug=False) -> np.ndarray:
        rbt_tcp_homomat = rm.homomat_from_posrot(*self._arm_x.get_pose())
        if toggle_debug:
            gm.gen_sphere(rbt_tcp_homomat[:3, 3], ).attach_to(base)
        trans = np.dot(rbt_tcp_homomat, self._hand_to_eye_mat)
    ...
    def scan(self, toggle=False):
        ...
        for i in range(len(self._obs_p_list)):
            ...
            suc = self._arm_x.move_p(pos=obs_pos, rot=np.array([0, -3.1415926, 0]), speed=500, mvacc=1500)
            pcd, pcd_color, _, im = self._sensor_hdl.get_pcd_texture_depth()
            pcd_w = self._to_w_coord(pcd)
    ...
    def track(self, rack_proto, rack_init_pose, height_range, rack_state=None, toggle_pcd=False):
