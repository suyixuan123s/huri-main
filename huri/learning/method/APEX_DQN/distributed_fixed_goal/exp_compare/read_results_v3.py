...
def plot_img(data_list,
    ...
    for i, df in enumerate(data_list):
        if toggle_label:
            plt.plot(df.keys(), df.values(), color=colors[i], label=labels[i], linewidth=5, linestyle=line_type[i])
        else:
            # plt.plot(df.keys(), df.values(), marker=markers[i], color=colors[i], linewidth=3)
            plt.plot(df.keys(), df.values(), color=colors[i], linewidth=5, linestyle=line_type[i])
    ...
    if toggle_y_max:
        y_position = max(data_list[0].values())
        plt.axhline(y=y_position, color='r', linestyle='--', linewidth=3)  # Green dash-dot horizontal line
        ax.text(1, y_position+0.09, f"{y_position:.2}s", fontsize=30, fontname='Times New Roman', color='r',
