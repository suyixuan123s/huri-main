...
def load_comparasion_data(path: fs.Path, path2: fs.Path = None):
    p = fs.Path(path)
    eval_path = list(p.glob('**/eval_results*pkl'))

    if path2 is not None:
        p = fs.Path(path2)
        eval_path2 = list(p.glob('**/eval_results*pkl'))
        if len(eval_path2) < 1:
    ...
    data = [success_rate(d3qn_solver_success_inds_dict),
            success_rate(astar_solver_success_inds_dict), ]
    toggle_label = False
    plot_success_rate(data, toggle_label=toggle_label)
    print("average time")
    data_time = [average_time(d3qn_solver_time_dict), ]
    ...
    data_time = [average_success_time(d3qn_solver_time_dict, astar_solver_success_inds_dict),
    ...
    data_time = [average_len(d3qn_solver_solution_dict), ]
    plot_average_len(data_time,
                     y_limit_max=None,
                     divide_y=None,
                     toggle_label=toggle_label)

    data_time = [average_success_len(d3qn_solver_solution_dict, astar_solver_success_inds_dict),
...
if __name__ == "__main__":
    ...
    # INDEX = 'b'
    load_comparasion_data(
        'E:\huri_shared\huri\learning\method\APEX_DQN\distributed_fixed_goal\exp_utils\exp_solution_A\A0',
        path2=r'E:\huri_shared\huri\learning\method\APEX_DQN\distributed_fixed_goal\exp_compare\data\5_10_3')
    # load_comparasion_data(
