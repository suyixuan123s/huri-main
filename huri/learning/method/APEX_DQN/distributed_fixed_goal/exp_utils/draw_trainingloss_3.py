...
if __name__ == '__main__':
    ...
    # labels = ['Baseline+R.+T.', 'Baseline+R.', 'Baseline+T.', 'Baseline', ]
    # -------------------------------
    training_levels = df['training_level'].unique()
    # Create a colormap
    cm = plt.get_cmap('viridis')
    # Create a color for each training level
    colors = cm(np.linspace(0, 1, len(training_levels)))

    xxx = 0
    for i, level in enumerate(training_levels):
        # Find the range for the current level
        level_data = df[df['training_level'] == level]
        xmin = level_data['train_steps'].min()
        xmax = level_data['train_steps'].max()
        # Shade the area for this level
        # plt.axvline(x=xmax, color=colors[i], linestyle='-', linewidth=2)
        plt.axvline(x=xmax, color=colors[i], linestyle='-', linewidth=1)
        plt.axvspan(xxx, xmax, color=colors[i % len(colors)], alpha=0.3, label=f'Training Level {level}')
        xxx = xmax
    # -------------------------------
    plt.plot(df['train_steps'], df['eval_average_score'], color="#0072B2",
             linewidth=5)
        # plt.axvspan(xmin, xmax, color=colors[i % len(colors)], alpha=0.3, label=f'Training Level {level}')
