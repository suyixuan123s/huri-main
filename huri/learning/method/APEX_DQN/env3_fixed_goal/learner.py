...
class HERLearner2(Learner):
    ...
    def _compute_dqn_loss(self, samples: Dict[str, np.ndarray], toggle_debug: bool = False) -> (
        ...
        # G_t   = r + gamma * v(s_{t+1})  if state != Terminal
        #       = r                       otherwise
        # state_feasible_action_set = torch.as_tensor(
        #     np.array([padding(action_dim,
        #                       env_to_state_func(obs_tmp.cpu().numpy()).feasible_action_set
        #                       ) for obs_tmp in samples["obs"]]),
        #     dtype=torch.int64, device=device)
        ...
        curr_q_table = self.dqn(state, goal)
        curr_q_value = curr_q_table.gather(1, action)
        # curr_q_table_v = curr_q_table.gather(1, state_feasible_action_set)

        # target_curr_q_table2 = self.dqn_target.forward2(state, goal).detach()
        # target_curr_q_table_v2 = target_curr_q_table2.gather(1, state_feasible_action_set)
    ...
    def update_model(self, toggle_debug=False) -> (torch.Tensor, torch.Tensor):
        """Update the model by gradient descent."""
        samples = self.replay_memory.sample_batch()
        weights = samples["weights"].reshape(-1, 1)
        indices = samples["indices"]
        ...
        self.replay_memory.update_priorities(indices, new_priorities)
        return loss.item()
        # return loss_arrange.item(), loss_permutation.item()
    ...
    def learn(self, T, create_agent, init_dqn_path: str = None, lr_update_step=2000):
        ...
        self.dqn_target.load_state_dict(self.dqn.state_dict())
        self.shared_net.load_state_dict(self.dqn.state_dict())
        self.replay_memory.update_beta(step=0)
        ...
        for t in range(T):
            ...
            if self.num_dqn_updates % lr_update_step == 0:
                if state_level != self.shared_state['state_level']:
                    state_level = self.shared_state['state_level']
                    self.env_to_state_func(np.array([1]))._cache.clear()
            ...
            if self.num_dqn_updates % output_interval == 0:
                print(
                    f"Learner update numbers: {self.num_dqn_updates}, "
                    f"Loss1 average: {loss1_total / output_interval:4f}, max loss: {max_loss1:4f}, " 
                    f"Time average is {time_average:2f} ms,"
    ...
    #
    #     torch.save({
    #         'q_state_dict': self.dqn.state_dict,
    #         'q_target_state_dict': self.dqn_target.state_dict,
