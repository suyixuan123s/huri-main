...
from huri.learning.method.APEX_DQN.env3.learner import Learner, HERLearner2
from huri.learning.method.APEX_DQN.env3.actor import Actor, Eval, HERActor
from huri.learning.method.APEX_DQN.env3.reanalyzer import Reanalyzer
from huri.learning.method.APEX_DQN.env3.diffcult_actor import DifficultActor
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eps-decay", type=float, default=1e-6, help="epsilon greedy decay rate")
    ...
    parser.add_argument("--update-freq", type=int, default=500, help="target network update frequency")
    ...
    shared_difficult_state_buffer_list = [mp.Queue() for _ in range(num_difficult_actors)]
    ...
    for i in range(num_difficult_actors):
        env_actor = env.copy()
        env_actor.set_seed(i * args.seed)
        # env_actor.scheduler.state_level = actor_params[i][0]
        # env_actor.scheduler.class_level = actor_params[i][1]
        difficult_actor_proc = DifficultActor(actor_id=i,
                                              net=copy.deepcopy(net),
                                              env=env_actor,
                                              epsilon_decay=args.eps_decay,
                                              max_epsilon=args.max_eps,
                                              ...
                                              shared_state=shared_state,
                                              shared_reanalyzer_mem=shared_reanalyzer_mem,
                                              difficult_case_buffer=shared_difficult_state_buffer_list[i],
                                              shared_mem=shared_mem,
                                              device=device,
                                              toggle_visual=False)
        difficult_actor_proc.start()
        LOGGER.info(f"Difficult Actor {i} Process: started")
        actor_procs.append(difficult_actor_proc)

    reanlyr_procs = []
    debug_set = [False, False, False, False, False, False, debug_visual_reanalyzer]
    ...
    eval_proc = Eval(net=copy.deepcopy(net),
                     ...
                     toggle_visual=False,
                     difficult_case_buffer_list=shared_difficult_state_buffer_list,
