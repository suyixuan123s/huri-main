...
from huri.learning.env.rack_v0.env import RackArrangementEnv, RackStatePlot, \
    StripeRackStateScheduler, GOAL_PATTERN_5x10, RackState
import numpy as np
...
def generate_training_data(args, toggle_debug=False):
    (state_level, iter_num, goal_pattern, env) = args
...
if __name__ == "__main__":
    ...
    with mp.Pool(num_actor) as p:
        training_data = sum(p.map(generate_training_data,
                                  [(20, int(each_state_data_num / num_worker_pool),
                                    GOAL_PATTERN_5x10, env.copy()), ] * num_worker_pool), [])
    fs.dump_pickle(training_data, path=f"demo_training_data_{rack_size[0]}_{rack_size[1]}.pkl")

    # data2 = p2.map(generate_training_data, [(2, 2, 1)] * 5)
    # print(data2)
    # proc_pool1 = []
    # proc_pool2 = []
    # for i in range(num_worker_each_pool):
    #     p1 = mp.Process(target=generate_training_data, args=(rss_easy, 29, 30000 / num_worker_each_pool))
    #     p2 = mp.Process(target=generate_training_data, args=(rss_diff, 29, 30000 / num_worker_each_pool))
    #     proc_pool1.append(p1)
    #     proc_pool1.append(p2)
