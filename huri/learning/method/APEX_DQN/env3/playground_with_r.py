...
from env_tst import env, num_tube_classes, rack_size, action_space_dim, observation_space_dim, RackState
import itertools
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        for _ in reversed(infeasible_data_list):
            ...
            refined_path = rm_ras_actions([np.array(ss) for ss in v_states],
                                                h=8,
                                                goal_pattern=np.array(v_states[-1]),
                                                infeasible_dict={}, )

            refined_path_to_transitions_recur(env,refined_path,goal_pattern=refined_path[-1])
            b = time.time()
            print("Time consumption is ", b - a)
            plot(v_states, np.array(goal))
            plot(refined_path, np.array(goal), w_name="2")
            cv2.waitKey(0)
