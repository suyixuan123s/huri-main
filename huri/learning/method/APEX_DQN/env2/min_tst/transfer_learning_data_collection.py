...
from huri.learning.env.rack_v2.env import RackArrangementEnv, RackStatePlot, \
    GoalRackStateScheduler, RackState
import numpy as np
import multiprocessing as mp
from os import getpid
from tqdm import tqdm

from env_tst import env, num_tube_classes, rack_size, action_space_dim
...
def generate_training_data(args, toggle_debug=False):
    ...
    rss = GoalRackStateScheduler(num_classes=num_tube_classes, rack_size=rack_size, state_level=1)
    train_data = []
...
if __name__ == "__main__":
    ...
    with mp.Pool(num_actor) as p:
        training_data = sum(p.map(generate_training_data,
                                  [(17, int(each_state_data_num / num_worker_pool), env.copy()), ] * num_worker_pool),
                            [])
