...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (state, goal, action, label, feasible_acts) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        q, feasible_act_feature = model(state, goal)
        feasible_act = feasible_act_decoder(feasible_act_feature)
        q_loss = F.mse_loss(q.gather(1, action.reshape(-1, 1)), label.reshape(-1, 1))
...
net = DuelingDQN5(obs_dim=observation_space_dim, action_dim=action_space_dim, num_classes=num_tube_classes).to(device)
...
net.encoder.load_state_dict(pre_trained_dict)
