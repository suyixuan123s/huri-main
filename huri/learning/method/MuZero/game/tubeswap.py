...
class MuZeroConfig:
    def __init__(self):
        ...
        self.device = select_device()  # Fix the maximum number of GPUs to use. It's usually faster to use a single GPU (set it to 1) if it has enough memory. None will use every GPUs available

        ### Game
        self.observation_shape = (1, 5,
                                  10)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
    ...
    def to_dict(self):
        return {
            ...
            'ubc_pb_c_base': self.pb_c_base,
            'ubc_pb_c_init': self.pb_c_init,
...
class RackArrangementEnvWrapper(object):
    def __init__(self, env):
        self.env = env

    @property
    def goal_pattern(self):
        return np.array(self.env.goal_pattern)

    def step(self, *args, **kwargs):
        next_obs, reward, done, _ = self.env.step(*args, **kwargs)
        return np.array(next_obs)[None,], reward, done, _
...
class Game(AbstractGame):
    ...
    def __init__(self,
                 ...
                 seed=777):
        self._env = RackArrangementEnv(rack_size=rack_size,
                                       num_classes=num_classes,
                                       observation_space_dim=obs_dim,
                                       action_space_dim=act_dim,
                                       is_curriculum_lr=is_curriculum_lr,
                                       is_goalpattern_fixed=is_goalpattern_fixed,
                                       seed=seed)
        self.scheduler = self._env.scheduler
        self.env = RackArrangementEnvWrapper(self._env)

    def get_goal_pattern(self):
        return self.env.goal_pattern
    ...
    def legal_actions_func(self, state: np.ndarray) -> list:
        """
        Should return the legal actions at each turn, if it is not available, it can return
        the whole action space. At each turn, the game have to be able to handle one of returned actions.
        ...
        """
        # TODO rewrite this part
        return RackState(state[0, :, :]).feasible_action_set.tolist()
