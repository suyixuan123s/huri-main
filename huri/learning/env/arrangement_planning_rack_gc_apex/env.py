...
class RackStatePlot:
    ...
    def plot_states(self, rack_states, row=6, plot_fillable=False, plot_movable=False):
        # length of the eack
        ...
        for state_ind, state in enumerate(rack_states):
            ...
            if plot_fillable or plot_movable:
                ...
                if plot_movable:
                    self._plot_cons(movable_slots, axe=axe_tmp, color="red")
                if plot_fillable:
                    self._plot_cons(fillable_slots, axe=axe_tmp, color="green")
        return Plot(fig=fig)
...
class RackArrangementEnv(Gym_Proto):
    ...
    def _get_reward(self, is_finished, state_current, state_next, goal_pattern):
        ...
        f = np.sum(self.F(state_next) - self.F(state_current))
    ...
    def evaluate_traj(self, paths, goal_pattern, toggle_debug=False):
        ...
        return paths, reward_traj, action_traj
...
class RackStateScheduler(object):
    def __init__(self, num_classes, rack_size, np_random=None, state_level=1, goal_level=1):
    ...
    def update_state_level(self):
        ...
        self.state_level = min(level, np.prod(self.rack_size))
    ...
    def __repr__(self):
        return f"Rack dimension is {self.rack_size}, current level is {self.state_level}"
...
if __name__ == "__main__":
    ...
    if is_find:
        env.evaluate_traj(path, goal, toggle_debug=True)
