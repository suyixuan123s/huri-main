...
import unittest
from huri.learning.env.rack_v3.env_mix import RackState, RackArrangementEnv, RackStatePlot
import numpy as np
...
class RackStateTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        print(f"obs dimension is {obs_dim}, action dimension is {act_dim}")
        self.env = RackArrangementEnv(rack_size=rack_size, num_classes=class_num, observation_space_dim=obs_dim,
                                      is_goalpattern_fixed=False, is_curriculum_lr=True, num_history=1)
        print("num of actions is", self.env.action_space_dim)
