...
def check_pattern_in_matrix(big_matrix, pattern):
    """
    Check if the given pattern exists in the big_matrix.
    """
    # Validate the input matrix and pattern
    if not isinstance(big_matrix, np.ndarray) or not isinstance(pattern, np.ndarray):
        raise ValueError("Both big_matrix and pattern must be numpy arrays")

    if big_matrix.ndim != 2 or pattern.ndim != 2:
        raise ValueError("Both big_matrix and pattern must be 2D matrices")

    if big_matrix.shape[0] < pattern.shape[0] or big_matrix.shape[1] < pattern.shape[1]:
        raise ValueError("big_matrix must be larger than or equal to the pattern in both dimensions")

    big_matrix_abs = (big_matrix > 0).astype(int)
    # Perform convolution
    result = ss.convolve2d(big_matrix_abs, pattern, mode='valid')

    # The pattern is found if the convolution result contains a value equal to the sum of the pattern
    return np.any(result == np.sum(pattern))
...
def extract_submatrix_mask(matrix, mask, extend_num=0):
    rows, cols = np.where(mask)

    if len(rows) == 0 or len(cols) == 0:
        return None  # No ones in the mask

    # Calculate the original bounds
    min_row, max_row = min(rows), max(rows) + 1
    min_col, max_col = min(cols), max(cols) + 1

    # Pad the matrix with zeros on all sides equal to extend_num
    padded_matrix = np.pad(matrix, pad_width=extend_num, mode='constant', constant_values=0)

    # Adjust the bounds for the padded matrix
    padded_min_row = min_row + extend_num
    padded_max_row = max_row + extend_num
    padded_min_col = min_col + extend_num
    padded_max_col = max_col + extend_num

    # Extract and return the extended submatrix
    return padded_matrix[padded_min_row - extend_num: padded_max_row + extend_num,
           padded_min_col - extend_num: padded_max_col + extend_num]
    ...
    while True:
        if min_obj_num == obj_num:
            ...
            if isinstance(np_random, np.random.Generator):
                num_random = np_random.integers(min_obj_num, obj_num)
            else:
