...
from huri.learning.env.rack_v1.utils import (get_possible_actions,
...
def to_action(rack_size, pick_id, place_id, state, action_space):
    place_id_int = place_id[:, 0] * rack_size[1] + place_id[:, 1]
    pick_id_int = pick_id[:, 0] * rack_size[1] + pick_id[:, 1]
    obj_class = state[(pick_id[:, 0], pick_id[:, 1])]
    return np.sort(place_id_int * np.prod(rack_size) + pick_id_int) + (obj_class - 1) * action_space
...
class RackState:
    ...
    def feasible_action_set(self):
        ...
        return to_action(rack_size, possible_actions[:, 0:2], possible_actions[:, 2:4], self.state,
...
class RackArrangementEnv(Gym_Proto):
    ...
    def _expr_action(self, action: int):
        ...
        action = action % self.action_space_dim
