...
class PickPlacePlanner(adp.ADPlanner):
    ...
    def find_common_graspids(self,
        ...
        failed_robotik_list = []
        if toggle_debug:
            debug_node = mc.ModelCollection()

        hnd_instance = self.robot_s.hnd_dict[hand_name].copy()
        # start reasoning
        ...
        for goalid, goal_homomat in enumerate(goal_homomat_list):
            ...
            graspid_and_graspinfo_list_upzip = list(graspid_and_graspinfo_list)
            # jnt_iks = get_available_graspids_ik(graspid_and_graspinfo_list_upzip, goal_pos, goal_rotmat, self.robot_s,
            #                                     hand_name,
            #                                     12)
            for idx, (graspid, grasp_info) in enumerate(graspid_and_graspinfo_list_upzip):
                # grasp info
                ...
                # use mesh collision
                # is_hnd_collided = hnd_instance.is_mesh_collided(obstacle_list)
                # use primitive collision
                # is_hnd_collided = hnd_instance.is_collided(obstacle_list)
                is_hnd_collided = False
                if not is_hnd_collided:  # hnd_s cd
                    jnt_values = self.robot_s.ik(hand_name, goal_jaw_center_pos, goal_jaw_center_rotmat)
                    # jnt_values = jnt_iks[idx]
                    if jnt_values is not None:  # common graspid with robot_s ik
                        if toggle_debug:
                            hnd_tmp = hnd_instance.copy()
                            hnd_tmp.gen_meshmodel(rgba=[0, 1, 0, .2]).attach_to(debug_node)
                        ...
                        if (not is_rbt_collided) and (not is_obj_collided):  # hnd cdfree, rbs ikf/cdfree, obj cdfree
                            ...
                            if toggle_debug:
                                self.robot_s.gen_meshmodel(rgba=[1, 0, 1, .5]).attach_to(debug_node)
                                failed_robotik_list.append(jnt_values)
                                print("cd elements len ", len(self.robot_s.cc.all_cdelements))
                    else:  # hnd_s cdfree, robot_s ik infeasible
                        ikfailed_grasps_num += 1
                        if toggle_debug:
                            hnd_tmp = hnd_instance.copy()
                            hnd_tmp.gen_meshmodel(rgba=[1, .6, 0, .2]).attach_to(debug_node)
                else:  # hnd_s collided
            ...
            print('Number of feasible robots at goal-' + str(goalid) + ': ', len(previously_available_graspids))
            print('------end------')
            # if goalid ==0  and len(previously_available_graspids) == 0:
            #     #
            #     for bad_ik in failed_robotik_list:
            #         self.robot_s.fk(hand_name,bad_ik)
            #         if not self.robot_s.is_collided([]):
            #             _, badpoints = self.robot_s.is_collided(obstacle_list, toggle_contact_points = True)
            #             for p in badpoints:
            #                 gm.gen_sphere(pos=p,radius=0.01).attach_to(base)
            #
            #
            #     debug_node.attach_to(base)
            ...
            #         obs.show_cdprimit()
            #     base.run()
            #     raise ReplanningFlag("replanning")
