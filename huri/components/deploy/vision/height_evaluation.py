...
class SelectRegion(SelectRegionBasePhoxiStreamer):
    ...
    def _init_gui(self):
        ...
        self._panel.add_button(text="Analyse Table Height", command=self._analyse_plane, pos=(_row, 1))
    ...
    def _analyse_plane(self):
        print("Analyse Plane")
        if self.selected_pcd is not None:
            segment_table_plane(self.selected_pcd, toggle_plane_model=True)
        else:
    ...
    def _update(self, x):
        ...
        if data is None:
            ...
            self.selected_pcd = self._pcd_aligned[extracted_pcd_idx] if self._pcd_aligned is not None else self._pcd[
                extracted_pcd_idx]
            gm.gen_pointcloud(self.selected_pcd, rgbas=[[1, 0, 0, 1]]).attach_to(self._np_pcd)
