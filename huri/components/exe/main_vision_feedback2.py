...
if __name__ == "__main__":
    ...
    debug_filename = fs.workdir / "data" / "vision_exp" / "20220201-002227.pkl"
    pcd, img = capture_vision_info(debug_filename=debug_filename,
                                   toggle_debug=DEBUG)  # TODO revise debug back to DEBUG
    ...
    while not isdone(node=tube_rack.rack_status, goalpattern=GOAL_PATTERN):
        ...
        for sol in task_sols:
            ...
            if start2pickapproach_motion_seg is not None:
                ...
                exe_motion_batch = MotionBatchPPP(
                    *[ppp.motion_seg_to_element(motion_seg,
                                                component_name,
                                                moved_tube_cm.copy(),
                                                obs_testubes_cmlist, )
                ...
                if not DEBUG:
                    ...
                    motion_executer(yumi_s=yumi_robot, yumi_con=yumi_con, motion_batch=remote_motion_batch,
                                    open_gripper_len=open_gripper_len, logger=exe_logger)
                    # Feedback
