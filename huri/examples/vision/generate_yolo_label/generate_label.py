...
def auto_labeling(yolo_weight_path,
    ...
    for idx, file_path in enumerate(tqdm(sampled_file_paths, desc='Scanning images', total=len(sampled_file_paths))):
        ...
        if file_path.name.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
            enhanced_image = cv2.imread(str(file_path), cv2_img_code)
        elif file_path.name.lower().endswith(('pkl', 'pickle')):
            pcd, img, depth_img, _, ext_cam_img = vision_read_data(file_path)
            enhanced_image = cv.equalizeHist(img)
        else:
            print("Unsupport format")
            continue
        if is_gray_img:
            enhanced_image = np.stack((enhanced_image,) * 3, axis=-1)
        yolo_img, yolo_results = yyd.detect(source=enhanced_image, weights=yolo_weight_path, cache_model=True)
        ...
        if save_label_path is not None:
            ...
            with open(str(save_label_path / f"{label_name}.txt"), "w") as f:
                for key, item in yolo_results.items():
                    tube_id = check_id(" ".join(key.split(" ")[:-1]), label_info)
                    pos = item['pos']
        ...
        if save_img_path is not None:
            cv.imwrite(str(save_img_path / f"{img_name}.jpg"), enhanced_image)
...
if __name__ == "__main__":
    ...
    label_info = {
        'rack 1': 0, 'blue': 1, 'purple': 2, 'white': 3, 'purple ring': 4, 'white cap small': 5
    }
