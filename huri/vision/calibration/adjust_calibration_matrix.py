...
def test(yumi_s, pcd, base):
    ...
    def adjust_pcd(affine_mat_node, pcd_node_node, pcd, task):
        ...
        if is_print:
            json_array = json.dumps(affine_mat_node[0].tolist())
            print("-" * 10)
            print(json_array)
...
if __name__ == "__main__":
    ...
    REAL_ROBOT = True
    AFFINE_MAT_PATH = fs.workdir / "data" / "calibration" / "affine_mat_20240326-154703.json"
    ###############################################################

    # Init base
    base = wd.World(cam_pos=[1.5, 0, 0.25], lookat_pos=[0, 0, 0.25],
                    lens_type="perspective")  # , lens_type="orthographic"
    # base = wd.World(cam_pos=[0, 0, 1.5], lookat_pos=[0, 0, 0] , lens_type="orthographic")  # , lens_type="orthographic"
    # base = wd.World(cam_pos=[1.5, 0, 0.25], lookat_pos=[0, 0, 0.25], lens_type="orthographic")  # , lens_type="orthographic"
    # base = wd.World(cam_pos=[0, 1.5, 0], lookat_pos=[0, 0, 0], lens_type="orthographic")  # , lens_type="orthographic"
