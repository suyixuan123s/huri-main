...
from constant import *  # import constants and logging conf
from rgb_phoxi_calib.utils import project_pcd2extcamimg
from utils.gui_utils import SelectRegionBasePhoxiStreamer, img_to_n_channel, phoxi_map_pcd_2_poly_mask, highlight_mask
...
if __name__ == "__main__":
    from constant import (TUBE_NAME,
                          SAVE_PATH,
                          WORK_ARM,
                          IMG_TYPE,
                          TOGGLE_DEBUG)
    from huri.core.common_import import wd, ym, rm, gm
    ...
    # generate white cap as the collision model for the robot mover
    try:
        tube = TubeType.gen_tube_by_name(TUBE_NAME)
    except:
        print(f"[WARNING] Tube name :{TUBE_NAME} is not previously defined. Use `white tube` "
              f"for collision detection during manipulation.")
        tube = TubeType.gen_tube_by_name('white cap')
    # setup robot mover
    ...
    al = SelectRegion(showbase=base,
                      ...
                      tube_label=TUBE_NAME,
                      pcd_affine_matrix=affine_mat,
                      work_armname=WORK_ARM,
