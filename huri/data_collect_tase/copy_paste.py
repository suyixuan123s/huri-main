...
from utils.copy_paste_utils import parse_collected_data
...
BG2K_PATH = fs.workdir.joinpath("paper_draw", "train")
...
def copy_paste_annotation(cp_paths_and_classes,
                          ...
                          save_path: fs.Path or str = "data2",
                          num_imgs=20,
                          num_tubes=20,
    ...
    path = fs.Path(BG2K_PATH)
    random_bg_sampled_file_paths = list(path.glob("*"))

    save_path = fs.Path(save_path)
    ...
    for _ in range(num_imgs):
        ...
        if not toggle_debug:
            ...
            cv2.imwrite(str(save_path.joinpath(f'images/{img_name}.jpg')), img)
            write_yolo_annotation_file(b, bboxes=annotations,
                                       save_path=save_path.joinpath(f'labels/{img_name}.txt'))
        if _ % 100 == 0:
...
if __name__ == "__main__":
    save_data_path = r'.\data\data_syn'
