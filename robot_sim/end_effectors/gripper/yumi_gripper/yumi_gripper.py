...
class YumiGripper(gp.GripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), cdmesh_type='convex_hull', name='yumi_gripper',
        ...
        self.lft.lnks[0]['collision_model'] = cm.CollisionModel(
            os.path.join(this_dir, "meshes", "base.stl"),
            cdprimit_type="cylinder", expand_radius=.001, )
        # userdefined_cdprimitive_fn=self._hnd_combined_cdnp)
        self.lft.lnks[0]['rgba'] = [.5, .5, .5, 1]
        self.lft.lnks[1]['name'] = "finger1"
        self.lft.lnks[1]['mesh_file'] = cm.CollisionModel(
            os.path.join(this_dir, "meshes", "yumi_finger.stl"),cdprimit_type = "user_defined",
        ...
        self.rgt.lnks[1]['mesh_file'] = cm.CollisionModel(
            os.path.join(this_dir, "meshes", "yumi_finger.stl"), cdprimit_type = "user_defined",
            userdefined_cdprimitive_fn=self._finger_cdnp, expand_radius=.000)
        # os.path.join(this_dir, "meshes", "yumi_finger.stl")
    ...
    def _finger_cdnp(name, radius):
        ...
        collision_primitive_c0 = CollisionBox(Point3(-.004, 0.006505, .02),
                                              x=.004+radius, y=0.005+ radius, z=.02+radius)
        collision_node.addSolid(collision_primitive_c0)
        collision_primitive_c1 = CollisionBox(Point3(-.0018, 0.006505, .052 - 0.009),
                                              x=.0018 + radius, y=0.0015 + radius, z=.015 + radius)
        collision_node.addSolid(collision_primitive_c1)
        return collision_node

    @staticmethod
    def _hnd_combined_cdnp(name, radius):
        collision_node = CollisionNode(name)
        collision_primitive_c0 = CollisionBox(Point3(0, 0, .035),
                                              x=.041096 + radius, y=0.035 + radius, z=.035 + radius)
        collision_node.addSolid(collision_primitive_c0)
        collision_primitive_c1 = CollisionBox(Point3(0, 0, .082),
                                              x=.02 + radius, y=0.025 + radius, z=.012 + radius)
        collision_node.addSolid(collision_primitive_c1)

        collision_primitive_c2 = CollisionBox(Point3(.03, 0, .082),
                                              x=.01 + radius, y=0.02 + radius, z=.012 + radius)
        collision_node.addSolid(collision_primitive_c2)

        collision_primitive_c3 = CollisionBox(Point3(-.03, 0, .082),
                                              x=.01 + radius, y=0.02 + radius, z=.012 + radius)
