...
class XArmLite6WRSGripper2(ri.RobotInterface):
...
if __name__ == '__main__':
    ...
    xarm = XArmLite6WRSGripper2(enable_cc=True)
    # rand_conf = xarm.rand_conf(component_name='arm')
    xarm.fk("arm", np.array([0, 0, 0, 0, 0, -np.pi / 2]))
    # xarm.fk('arm', np.array([38.22271062, 13.96141209, 76.389571, 58.8175958, 68.01321151, 4.71238898]))
    from huri.core.base_boost import boost_base

    base = boost_base(base)
    xarm_meshmodel = [xarm.gen_meshmodel(toggle_tcpcs=False)]
    xarm_meshmodel[0].attach_to(base)


    def j1():
        xarm_meshmodel[0].remove()
        jnt = xarm.get_jnt_values("arm")
        jnt[0] += np.radians(5)
        xarm.fk("arm", jnt)
        xarm_meshmodel[0] = xarm.gen_meshmodel(toggle_tcpcs=True)
        xarm_meshmodel[0].attach_to(base)
        print(xarm.get_jnt_values("arm"))
    ...
    base.boost.bind_task_2_key("a", j1)
    base.boost.bind_task_2_key("s", j2)
    base.boost.bind_task_2_key("d", j3)
    base.boost.bind_task_2_key("f", j4)
    base.boost.bind_task_2_key("g", j5)
    base.boost.bind_task_2_key("h", j6)

    base.boost.bind_task_2_key("z", lambda: base.boost.screen_shot(f"{time.strftime('%Y%m%d-%H%M%S')}"))
    # xarm.fk("arm", np.array([]))
