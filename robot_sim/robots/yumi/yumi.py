...
try:
    from robot_sim.robots.yumi.trac_ik.trac_ik import TracIK
...
class Yumi(ri.RobotInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='yumi', enable_cc=True):
        ...
        # self.lft_body.lnks[0]['mesh_file'] = os.path.join(this_dir, "meshes", "yumi_tablexyz.stl")
        self.lft_body.lnks[0]['mesh_file'] = os.path.join(this_dir, "meshes", "yumi_tablecovered.stl")
        self.lft_body.lnks[0]['rgba'] = [.55, .55, .55, 1.0]
        # self.lft_body.lnks[0]['collision_model'] = cm.CollisionModel(
        #     os.path.join(this_dir, "meshes", "yumi_tablenotop.stl"),
        #     cdprimit_type="user_defined", expand_radius=.005,
        #     userdefined_cdprimitive_fn=self._table_xyz_cdnp)
        # self.lft_body.lnks[0]['collision_model'] = cm.CollisionModel(
        #     os.path.join(this_dir, "meshes", "yumi_tablecovered.stl"), expand_radius=.03)
        ...
        # trac ik
        self.trac_ik_manipulator_dict = {}
        if is_trac_ik:
            # get the root of this file
            directory = os.path.abspath(os.path.dirname(__file__))
            yumi_urdf = os.path.join(directory, "trac_ik", "yumi3.urdf")
            self._ik_solver_rgt = TracIK("yumi_body", "yumi_link_7_r", yumi_urdf)
            self._ik_solver_lft = TracIK("yumi_body", "yumi_link_7_l", yumi_urdf)
        else:
            self._ik_solver_rgt = None
            self._ik_solver_lft = None
        self.trac_ik_manipulator_dict['rgt_arm'] = self._ik_solver_rgt
        self.trac_ik_manipulator_dict['lft_arm'] = self._ik_solver_lft
    ...
    @staticmethod
    def _table_xyz_cdnp(name, radius):
        collision_node = CollisionNode(name)
        collision_primitive_c0 = CollisionBox(Point3(-.08, 0, -.5075),
                                              x=.217 + radius, y=.6 + radius, z=.5075 + radius)
        collision_node.addSolid(collision_primitive_c0)
        collision_primitive_c1 = CollisionBox(Point3(.52, -.57, -.5075),
                                              x=.383 + radius, y=.03 + radius, z=.5075 + radius)
        collision_node.addSolid(collision_primitive_c1)
        collision_primitive_c2 = CollisionBox(Point3(.52, .57, -.5075),
                                              x=.383 + radius, y=.03 + radius, z=.5075 + radius)
        collision_node.addSolid(collision_primitive_c2)

        collision_primitive_c3 = CollisionBox(Point3(.873, 0, -.5075),
                                              x=.03 + radius, y=.6 + radius, z=.5075 + radius)
    ...
    #         component_name = "rgt_arm"
    #     offline_data_arm = self.offline_data.get(component_name, None)
    ...
    #         tgt_vec = voxelize(np.concatenate((tgt_pos, tgt_rotmat[:3, 2])).reshape(1, -1), voxel_size_pos, voxel_size_rot)
    #         # a = time.time()
    ...
    #             self.fk(component_name, init_ik)
    #             _, rot = self.get_gl_tcp(component_name)
    #             init_ik[-1] = -rm.angle_between_vectors(rot[:3, 0], tgt_rotmat[:3, 0])
    #         return self.manipulator_dict[component_name].ik(tgt_pos,
    #                                                         tgt_rotmat,
    #                                                         seed_jnt_values=seed_jnt_values,
    #                                                         max_niter=max_niter,
    ...
    def ik(self,
           ...
           local_minima="accept",
           toggle_debug=False):
        if self._ik_solver_rgt is None or self._ik_solver_lft is None:
            return super().ik(component_name, tgt_pos, tgt_rotmat, seed_jnt_values, max_niter, tcp_jntid,
                              tcp_loc_pos, tcp_loc_rotmat, local_minima, toggle_debug)
        else:
            # TODO remove the offset in the IRB14050
            if "rgt" in component_name:
                seed_jnt_values = self.rgt_arm.homeconf if seed_jnt_values is None else seed_jnt_values.copy()
                tgt_pos = tgt_pos - np.dot(tgt_rotmat, (self.rgt_hnd.jaw_center_pos + np.array([0, 0, 0.007])))
                return self._ik_solver_rgt.ik(tgt_pos, tgt_rotmat, seed_jnt_values=seed_jnt_values)
            elif "lft" in component_name:
                seed_jnt_values = self.lft_arm.homeconf if seed_jnt_values is None else seed_jnt_values.copy()
                tgt_pos = tgt_pos - np.dot(tgt_rotmat, (self.lft_hnd.jaw_center_pos + np.array([0, 0, 0.007])))
                return self._ik_solver_lft.ik(tgt_pos, tgt_rotmat, seed_jnt_values=seed_jnt_values)
            else:
                raise Exception("!")
...
if __name__ == '__main__':
    ...
    # yumi_meshmodel = yumi_instance.gen_stickmodel(toggle_tcpcs=True, toggle_jntscs=True,toggle_connjnt=True)
    # yumi_instance.fk("rgt_hnd", np.zeros(7))
    # yumi_instance.fk("lft_hnd", np.zeros(7))
    from numpy import array

    # pos = array([    0.25416,    -0.16866,     0.24629])
    # rot = array([[   0.043619,     0.99768,    0.052286],
    #    [    0.99905,    -0.04356,  -0.0022829],
    #    [ 4.3368e-19,    0.052336,    -0.99863]])
    # seed_jnt = array([   -0.34907,     -1.5708,     -2.0944,      0.5236,           0,     0.69813,           0])
    # gm.gen_frame(pos, rot).attach_to(base)
    # for i in range(1):
    #     print("ik result", yumi_instance.ik("rgt_arm", pos, rot, seed_jnt_values=seed_jnt))
    # yumi_instance.fk("rgt_arm", yumi_instance.ik("rgt_arm", pos, rot, seed_jnt_values=seed_jnt))
    yumi_instance.gen_meshmodel().attach_to(base)
    print(yumi_instance.is_collided())
    base.run()

    yumi_instance.fk("rgt_arm", np.array([1.1943, -1.5112, -1.4419, 0.8009, 0.60134, -1.3614, 2.4285]))
    print(yumi_instance.get_gl_tcp("rgt_arm"))
    exit(0)
    yumi_meshmodel = yumi_instance.gen_meshmodel(toggle_jntscs=False)
