...
class Nova2(mi.ManipulatorInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(6), name='nova2', enable_cc=True):
        ...
        self.jlc_mount = jl.JLChain(pos=self.jlc.jnts[-1]['gl_posq'],
                                    rotmat=self.jlc.jnts[-1]['gl_rotmatq'],
                                    homeconf=np.zeros(2), name="tcp mount")

        # # # zivid mount
        # self.jlc_mount.lnks[0]['name'] = "zivid_mount"
        # self.jlc_mount.lnks[0]['loc_pos'] = np.zeros(3)
        # self.jlc_mount.lnks[0]['mesh_file'] = cm.CollisionModel(os.path.join(this_dir, "meshes", "zivid_mount.stl"),
        #                                                         cdprimit_type="user_defined",
        #                                                         userdefined_cdprimitive_fn=self._zivid_mount_cdnp)
        # self.jlc_mount.lnks[0]['rgba'] = [.57, .57, .57, 1.0]
        # # zivid cam
        # self.jlc_mount.lnks[1]['name'] = "zivid_cam"
        # self.jlc_mount.lnks[1]['loc_pos'] = np.array([0, -0.064898 * 1.55, 0])
        # self.jlc_mount.lnks[1]['mesh_file'] = os.path.join(this_dir, "meshes", "zivid_cam.stl")
        # self.jlc_mount.lnks[1]['rgba'] = [.57, .57, .57, 1.0]
        # # hand
        # self.jlc_mount.lnks[2]['name'] = "hand"
        # self.jlc_mount.lnks[2]['loc_pos'] = np.array([0,-.2,0])
        # self.jlc_mount.lnks[2]['mesh_file'] = os.path.join(this_dir, "meshes", "hand.stl")
        # self.jlc_mount.lnks[2]['rgba'] = [.57, .57, .57, 1.0]
        # self.jlc_mount.reinitialize()
        # collision detection
        if enable_cc:
            self.enable_cc()
    ...
    @staticmethod
    def _zivid_mount_cdnp(name, radius):
        collision_node = CollisionNode(name)
        collision_primitive_c0 = CollisionBox(Point3(0, -0.02, 0.004),
                                              x=.035 + radius, y=.06 + radius, z=0.008 + radius)
        collision_node.addSolid(collision_primitive_c0)
        # collision_primitive_c1 = CollisionBox(Point3(0, 0, .124),
        #                                       x=.043 + radius, y=.043 + radius, z=.049 + radius)
        # collision_node.addSolid(collision_primitive_c1)
    ...
    def enable_cc(self):
        ...
        # self.cc.add_cdlnks(self.jlc_mount, [0, 1, 2])
        # fromlist = [self.jlc_mount.lnks[0],
        #             self.jlc_mount.lnks[1],
        #             self.jlc_mount.lnks[2]]
        # intolist = [self.jlc.lnks[0],
        #             self.jlc.lnks[1],
        #             self.jlc.lnks[2],
        #             self.jlc.lnks[3],
        #             self.jlc.lnks[4], ]
    ...
    def fk(self, jnt_values):
        value = self.jlc.fk(jnt_values=jnt_values)
        # self.jlc_mount.fix_to(pos=self.jlc.jnts[-1]['gl_posq'],
        #                       rotmat=self.jlc.jnts[-1]['gl_rotmatq'])
        return value

    def fix_to(self, pos, rotmat, jnt_values=None):
        value = self.jlc.fix_to(pos=pos, rotmat=rotmat, jnt_values=jnt_values)
        # self.jlc_mount.fix_to(pos=self.jlc.jnts[-1]['gl_posq'],
        #                       rotmat=self.jlc.jnts[-1]['gl_rotmatq'])
        return value
    ...
    def gen_meshmodel(self,
        ...
        self.jlc.gen_meshmodel(tcp_loc_pos=None,
                               tcp_loc_rotmat=None,
                               toggle_tcpcs=False,
                               toggle_jntscs=toggle_jntscs,
                               rgba=rgba).attach_to(mm_collection)
        # self.jlc_mount.gen_meshmodel(tcp_loc_pos=None,
        #                              tcp_loc_rotmat=None,
        #                              toggle_tcpcs=False,
        #                              toggle_jntscs=toggle_jntscs,
        #                              rgba=rgba).attach_to(mm_collection)
...
if __name__ == '__main__':
    ...
    manipulator_instance.fix_to(np.array([0, 0.3, 0.1]), np.eye(3))
