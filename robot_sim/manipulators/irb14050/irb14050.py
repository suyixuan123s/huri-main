...
class IRB14050(mi.ManipulatorInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(7), name='irb14050', enable_cc=True):
        ...
        # TODO Add a new jnts for the fixed joint (0.007 is the offset)
        self.jlc.jnts[7]['loc_pos'] = np.array([0.027, 0.029 + 0.007, 0.0]) # compensate the offset
...
if __name__ == '__main__':
    ...
    #
    # manipulator_meshmodel = manipulator_instance.gen_stickmodel(toggle_tcpcs=True)
    # manipulator_meshmodel.attach_to(base)
    # cm_list = manipulator_instance.gen_meshmodel().cm_list
    # for cm in cm_list:
    #     cm.attach_to(base)
    #     rgba = cm.get_rgba()
    #     cm.set_rgba([*rgba[:3], .3])
    # base.run()
    ...
    manipulator_instance2 = IRB14050(enable_cc=True)
    ...
    cm_list = manipulator_meshmodel2.cm_list
    for cm in cm_list:
        cm.attach_to(base)
        rgba = cm.get_rgba()
        cm.set_rgba([*rgba[:3], .3])
    ...
    manipulator_instance2.gen_stickmodel(toggle_jntscs=True).attach_to(base)
    base.run()
