...
def __draw_registration_result(source_o3d, target_o3d, transformation):
    ...
    if source_tmp[0] is not None:
        source_tmp[0].remove()
    source_tmp[0] = gm.gen_pointcloud(
        rm.homomat_transform_points(transformation, np.asarray(source_temp.points)),
        [[1, 0.706, 0, 1]])
    source_tmp[0]._objpdnp.reparentTo(base.render2)

    if target_tmp[0] is not None:
        target_tmp[0].remove()
    target_tmp[0] = gm.gen_pointcloud(
        np.asarray(target_temp.points),
        [[0, 0.651, 0.929, 1]])
    target_tmp[0]._objpdnp.reparentTo(base.render2)
...
def registration_ptpln(src, tgt, downsampling_voxelsize=.003, toggledebug=False):
    """
    registrate two point clouds using global registration + local icp
...
def remove_outlier(src_nparray, downsampling_voxelsize=2, nb_points=7, radius=3, estimate_normals=False,
    ...
    cl, ind = _removeoutlier_o3d(src_o3d, downsampling_voxelsize, nb_points, radius, toggledebug)
    return pda.o3dpcd_to_nparray(cl, return_normals=estimate_normals), ind
...
def _removeoutlier_o3d(src_o3d, downsampling_voxelsize=2, nb_points=7, radius=3, toggledebug=False):
    ...
    cl, ind = src_o3d_down.remove_radius_outlier(nb_points=nb_points, radius=radius)
    if toggledebug:
