...
class YuMiArm:
    ...
    def ik(self, pose, ext_axis, raw_res=False):
        '''Get the inverse kinematics of this arm to base frame of the arm.
        ...
        body = '{0}{1}'.format(YuMiArm._get_pose_body(pose), ext_axis if ext_axis is not None else '')
        req = YuMiArm._construct_req('ik', body)
        res = self._request(req, True)
    ...
    def set_speed_max(self, wait_for_res=True):
        """ Set the maximum speed of the robot_s to the maximum speed defined in YuMiConstants"""
    ...
    def goto_pose(self, pose, linear=True, relative=False, ext_axis=None, wait_for_res=True):
        ...
        relative : bool, optional
            ...
            Defaults to False
        ext_axis : string, optional
            If given, will use the given external axis in RAPID.
            Defaults to None
        wait_for_res : bool, optional
            If True, will block main process until response received from RAPID server.
            Defaults to True
        ...
        if relative:
            ...
            body = YuMiArm._get_pose_body(pose)
            body += f'{ext_axis:.2f}' if ext_axis is not None else ''
            if linear:
    ...
    def buffer_j_move(self, wait_for_res=True):
        ...
        res = self._request(req, wait_for_res, timeout=self._motion_timeout)
        return None if not wait_for_res else bool(int(res.message))
